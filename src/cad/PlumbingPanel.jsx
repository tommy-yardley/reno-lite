...
export default function PlumbingPanel({ cad }) {
  ...
  return <section>
    ...
    <div className="space-y-2 rounded-lg border border-[#D8CCB0] bg-[#FBF8F1] p-3">
      <div className="grid grid-cols-[1fr_92px] gap-2"><select value={system} onChange={(event) => { const next = event.target.value; setSystem(next); setDiameterMm(PIPE_SYSTEMS[next].defaultDiameterMm); }} className="rounded border border-[#D8CCB0] bg-transparent px-2 py-1.5 text-[10px]">{Object.entries(PIPE_SYSTEMS).map(([key, value]) => <option key={key} value={key}>{value.label}</option>)}</select><label className="text-[9px] text-[#8A97A3]">Pipe diameter<select value={diameterMm} onChange={(event) => setDiameterMm(Number(event.target.value))} className="mono mt-0.5 w-full rounded border border-[#D8CCB0] bg-transparent px-1 py-1 text-[10px]">{PIPE_SYSTEMS[system].diametersMm.map((value) => <option key={value} value={value}>Ø{value} mm</option>)}</select></label></div>
    ...
    <p className="mt-2 text-[9px] leading-4 text-[#8A97A3]">Concept routing only. Pipe sizing, falls, ventilation, isolation and connection details require a qualified plumber and site verification.</p>
